...
const each = (fn, arr) => arr?.forEach(fn);
const keys = (obj) => objectKeys(obj ?? {});
...
// Flatten syntax object
const syntaxFlat = {};
for (const category of Object.values(syntax)) {
  for (const [nodeName, node] of Object.entries(category)) {
...
for (const [key, val] of Object.entries(complexTypes)) {
...
// Alias map
const aliasMap = {};
for (const [nodeName, node] of Object.entries(syntaxFlat)) {
  if (node.alias) {
    aliasMap[node.alias] = nodeName;
  }
}

// Matches map - groups of node types
const matchesMap = {
...
const literalMap = {};
for (const [key, val] of Object.entries(literals)) {
  literalMap[key] = val;
  literalMap[val] = val;
}

// Attribute mapping (shorthand to full names)
const attrMap = {
  exp: 'expression',
  exps: 'expressions',
  then: 'consequent',
...
// Inverse attribute map
const attrMapInverse = {};
for (const [alias, name] of Object.entries(attrMap)) {
  if (!attrMapInverse[name]) {
    attrMapInverse[name] = [];
  }
  attrMapInverse[name].push(alias);
}

// Calculate primitive and non-primitive attributes
const primitiveAttributesSet = {};
const nonPrimitiveAttributesSet = {};

for (const node of Object.values(syntaxFlat)) {
  if (node.primitives) {
    each(attr => primitiveAttributesSet[attr] = true, node.primitives);
  }
  if (node.nodes) {
    each(attr => nonPrimitiveAttributesSet[attr] = true, node.nodes);
  }
  if (node.nodeArrays) {
    each(attr => nonPrimitiveAttributesSet[attr] = true, node.nodeArrays);
  }
}

const nonPrimitiveAttributes = keys(nonPrimitiveAttributesSet);
const primitiveAttributes = keys(primitiveAttributesSet);
const eitherAttributes = intersection(primitiveAttributes, nonPrimitiveAttributes);
